    ...
    def _generate_keys(difficulty: int) -> Tuple[bytes, bytes]:
        ...
        while True:
            ...
            if KeysAuth.is_pubkey_difficult(pub_key, difficulty):
                break
            # ugly, but lets be responsive on ^C (KeybordInterrupt)
            if reactor._started and reactor._stopped:  # noqa pylint: disable=protected-access
                raise Exception("reactor stopped")
